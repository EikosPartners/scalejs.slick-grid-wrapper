...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    var defaults = {
      ...
      enableFocusSinkTabStops: true, //ES 6/25/2015: Added to allow skipping the entire slickgrid object during tabbing but still allow cell navigation when clicked inside grid
      ...
      disableHorizontalOverflow: false, //ES 7/10/2015: Added to allow disabling of horizontal scroll bar in the grid
    ...
    function init() {
      ...
      $focusSink = $("<div tabIndex='" + (options.enableFocusSinkTabStops ? "0" : "-1") + "' hideFocus style='position:fixed;width:0;height:0;top:0;left:0;outline:0;'></div>").appendTo($container);
      ...
      $viewport.mousedown(function (e) {
        if ($viewport.is(e.target)) trigger(self.onScrollMouseDown, { args: e, grid: self });
      });

      $viewport.mouseup(function (e) {
        if ($viewport.is(e.target)) trigger(self.onScrollMouseUp, { args: e, grid: self });
      });

      $viewport.mousewheel(function (e) {
        trigger(self.onMouseWheel, { args: e, grid: self });
      });
      //ADDED BY BD - END

      if (options.disableHorizontalOverflow) $viewport.css("overflow-x", "hidden");
    ...
    function createColumnHeaders() {
      ...
      //Notifiy the header has been rendered
      trigger(self.onHeaderRowRendered, {
        "headerRow": getColumnHeaderRow(),
        "columns": columns
      });
    }
    ...
    function measureCellPaddingAndBorder() {
      ...
      //TODO: Determine why slickgrid or jquery determins paddingLeft and paddingRight as both 0px
    ...
    function createCssRules() {
      $style = $("<style type='text/css' rel='stylesheet' />").appendTo($(document.head));
    ...
    function getColumnCssRules(idx) {
      if (!stylesheet) {
        var sheets = document.getElementsByTagName("style"); // document.styleSheets;
        ...
        if (removedRowHash) {
          for (columnId in removedRowHash) {
            if (!addedRowHash || removedRowHash[columnId] != addedRowHash[columnId]) {
              ...
              if (node) {
                //console.log("before remove: " + node.className);
                $(node).removeClass(removedRowHash[columnId]);
                //console.log("after remove: " + node.className);
          ...
          for (columnId in addedRowHash) {
            if (!removedRowHash || removedRowHash[columnId] != addedRowHash[columnId]) {
              ...
              if (node) {
                ...
                //console.log("before add: " + node.className);
    ...
    function setCellCssStyles(key, hash) {
      ...
      trigger(self.onCellCssStylesChanged, { "key": key, "hash": hash, "prevHash": prevHash });
    }
    ...
    function getColumnHeaders() {
      return $headers.find(".slick-header-column").toArray();
    }
    ...
    $.extend(this, {
      ...
      "onScrollMouseDown": new Slick.Event(),
      "onScrollMouseUp": new Slick.Event(),
      ...
      // Manual Drag-drop and resize support additions
      "onHeaderRowRendered": new Slick.Event(),
