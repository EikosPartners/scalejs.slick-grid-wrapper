...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */(function(Buffer) {//Renamed to deepClone to avoid collisions - node it goes shallow on function objects (copies references only)
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _linq = __webpack_require__(13);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _linq = __webpack_require__(14);
...
/***/ function(module, exports, __webpack_require__) {
	...
	(function (root, undefined) {
	    ...
	    // module export
	    if ("function" === Types.Function && __webpack_require__(15)) { // AMD
...
/***/ function(module, exports, __webpack_require__) {
	...
	function setupFilter(fieldFilter, column, postFilterCallback) {
	    var internalFilter = observable([]),
	        //created local filter observable so it can be initialize it before grid is initialized.
	    templateName,
	        templateHasOptions,
	        throttleSpeed = 1,
	        //Zeroed at clients request but use 1 ms for performance on the select all button
	    quickFilter = observable(""),
	        //Filter box on the main grid
	    ...
	    function setIncomingQuickSearch(parentQuickSearch, surpressSettingQuickFilter) {
	        ...
	        if (!parentQuickSearch || !parentQuickSearch.values || !parentQuickSearch.values.length) {
	            quickSearch("");
	            if (!surpressSettingQuickFilter) quickFilter("");
	        } else {
	            quickSearch(parentQuickSearch.values[0]);
	            if (!surpressSettingQuickFilter) quickFilter(parentQuickSearch.values[0]);
	        }
	    ...
	    function setIncomingFilter(parentFilter) {
	        ...
	        if (parentFilter != null) value = _knockout2.default.isObservable(parentFilter) ? parentFilter() : parentFilter;
	        ...
	        var qsfilter = value.firstOrDefault(function (val) {
	            return val.isQuickSearch;
	        }, null);
	        ...
	        value.forEach(function (filter, index) {
	            if (filter.isQuickSearch) {
	                ...
	                if (index === 0) {
	                    ...
	                    if (filter.logicOperator != null) valueFields.logicOperator(filter.logicOperator);
	                }
	            }
	        });
	        ...
	        if (sendExpressionThrottled == null) sendExpressionThrottled = computed(function () {
	    ...
	    function onQuickSearch(filterText) {
	        ...
	        fieldFilter.quickSearch((0, _coreFunctions.has)(filterText) ? { op: quickOp, values: [filterText] } : undefined);
	    ...
	    function onListItems(newItems) {
	        ...
	        if (internal.length > 0) {
	            var filter = internal.firstOrDefault(function (f) {
	                return f.op === "In";
	            });
	            if (filter != null) checkedValues = filter.values;

	            filter = internal.firstOrDefault(function (f) {
	                return f.op === "StartsWith" && f.tag == null;
	            });
	            if (filter != null) quickfilter = filter.values[0];
	        }
	    ...
	    function sendExpression() {
	        ...
	        if (valueFields.valA() != null && valueFields.valA() !== "") expression.push({
	            op: valueFields.a(),
	            values: [valueFields.valA()],
	            tag: "valA"
	        });

	        if (valueFields.valB() != null && valueFields.valB() !== "") expression.push({
	            op: valueFields.b(),
	        ...
	        if (notEmpty()) expression.push({ op: "NotEmpty", values: [] });
	        ...
	        if (quickFilter() != null && quickFilter() !== "") {
	            ...
	            if (!templateHasOptions) {
	                ...
	                if (qs != "") expression.push({ op: quickOp, values: [quickSearch()], isQuickSearch: true });
	            } else {
	                ...
	                if (selectedList.length < selectableList.length || selectedList.length === selectableList.length && (qs != "" || isValFilter)) expression.push({ op: 'In', values: selectedList, quickSearch: qs });
	            }
	        }
	        //Send it in
	    ...
	    function calculateQuickFilter(suppressHandler) {
	        ...
	        if (suppressHandler) subscription.quickFilter.dispose();
	        ...
	        if (valueFields.valA() != null && valueFields.valA() !== "" || valueFields.valB() != null && valueFields.valB() !== "") {
	            ...
	            if (qs === "") {
	                ...
	                var matchedItems = selectableItems.where(function (item) {
	                    return item.toLowerCase().lastIndexOf(qslower, 0) === 0;
	                }).toArray();
	                checkedItems.length > 0 && JSON.stringify(checkedItems) === JSON.stringify(matchedItems) ? quickFilter(qs) : quickFilter("");
	            }
	        }
	        if (suppressHandler) subscription.quickFilter = quickFilter.subscribe(onQuickFilter);
	    }
	    ...
	    function getSelectedItems(valuesOnly) {
	        ...
	        for (var i = 0, j = 0; i < selectableList.length; i += 1) {
	            if (selectableList[i].selected) list[j++] = valuesOnly ? selectableList[i].value : selectableList[i];
	    ...
	    function createCheckboxOption(value, selected) {
	        return {
	            selected: (0, _coreFunctions.has)(selected) ? selected : true,
	            value: (0, _coreFunctions.has)(value) ? value.toString() : ""
	        };
	    }
	    ...
	    function initializeFilter($node) {
	        ...
	        $filter = (0, _jquery2.default)($node.find('.slick-filter')[0]);
	        $filter.click(function () {
	            ...
	            popupWindow.addEventListener("renderable", function (body) {
	                $popup = (0, _jquery2.default)('<div class="slick-filter-popup" data-bind="template: { name: popupTemplate, data: $data}"></div>').appendTo(body);
	                _knockout2.default.applyBindings(bindings, $popup.get()[0]);
	                ...
	                function setFilterFlip() {
	                    ...
	                    if (currmon != null && parentBounds.left + $filter.offset().left + $popup.width() + 20 > currmon.availableRect.right && !(parentBounds.left + $filter.offset().left - $popup.width() - column.width < currmon.availableRect.left))
	                    ...
	                    return flip ? $filter.offset().left - $popup.width() - column.width + 10 : $filter.offset().left + 20;
	                }
	                ...
	                $box = (0, _jquery2.default)('<div style="z-index: 5; position: absolute; left: 0px; top: 0px; width: 100%; height: 100%;"></div>').appendTo(doc.body);
	                $box.one("click", function () {
	                    $box.remove();
	                    popupWindow.close();
	                });

	                $arrow = (0, _jquery2.default)($popup.find('div')[0]);
	                $arrow.appendTo($box);
	                //$arrow.css("z-index", "5");
	                ...
	                if (currmon == null)
	                    ...
	                    positionTop = 10;else if (positionTop < currmon.availableRect.top)
	                    //Prevent the top of the popup from going off the screen
	                    positionTop = currmon.availableRect.top + 10;else if (positionTop > currmon.availableRect.top && positionTop + $popup.height() > currmon.availableRect.bottom)
	            ...
	            popupWindow.addEventListener("closed", function () {
	                ...
	                _knockout2.default.cleanNode($popup.get()[0]);
	    ...
	    function init(grid) {
	        grid.onHeaderRowCellRendered.subscribe(function (e, args) {
	            var $node = (0, _jquery2.default)(args.node),
	                ...
	                filterHtml = '<input type="text" data-bind="value: value, valueUpdate: \'afterkeydown\'"/>' + '<div class="slick-filter" data-bind="text: filterCountText, css: { iconFilterOff: !filterOn(), iconFilterOn: false, filterOn: filterOn }"></div>';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _jqueryEvent = __webpack_require__(21);
...
/***/ function(module, exports, __webpack_require__) {
	...
	"use strict";/*** IMPORTS FROM imports-loader ***/var slickCore=__webpack_require__(23);var jQuery=__webpack_require__(4);var $=__webpack_require__(4);var jqueryEventDragWrapper=__webpack_require__(20);(function(){/**
	 * @license
	...
	if(typeof jQuery==="undefined"){throw"SlickGrid requires jquery module to be loaded";}if(!jQuery.fn.drag){throw"SlickGrid requires jquery.event.drag module to be loaded";}if(typeof Slick==="undefined"){throw"slick.core.js not loaded";}(function($){// Slick.Grid
	$.extend(true,window,{Slick:{Grid:SlickGrid}});// shared across all grids on the page
	var scrollbarDimensions;var maxSupportedCssHeight;// browser's breaking point
	//////////////////////////////////////////////////////////////////////////////////////////////
	...
	/**
	   ...
	   **/function SlickGrid(container,data,columns,options){var document=container.ownerDocument;// settings
	var defaults={explicitInitialization:false,rowHeight:25,defaultColumnWidth:80,enableAddRow:false,leaveSpaceForNewRows:false,editable:false,autoEdit:true,enableCellNavigation:true,enableFocusSinkTabStops:true,//ES 6/25/2015: Added to allow skipping the entire slickgrid object during tabbing but still allow cell navigation when clicked inside grid
	enableColumnReorder:true,asyncEditorLoading:false,asyncEditorLoadDelay:100,forceFitColumns:false,enableAsyncPostRender:false,asyncPostRenderDelay:50,autoHeight:false,editorLock:Slick.GlobalEditorLock,showHeaderRow:false,headerRowHeight:25,showTopPanel:false,topPanelHeight:25,formatterFactory:null,editorFactory:null,cellFlashingCssClass:"flashing",selectedCellCssClass:"selected",multiSelect:true,enableTextSelectionOnCells:false,dataItemColumnValueExtractor:null,fullWidthRows:false,multiColumnSort:false,defaultFormatter:defaultFormatter,forceSyncScrolling:false,disableHorizontalOverflow:false,//ES 7/10/2015: Added to allow disabling of horizontal scroll bar in the grid
	disableColumnPicker:false//ES 6/1/2016: Added to allow surpressing of the column picker in grid-slick
	};var columnDefaults={name:"",resizable:true,sortable:false,minWidth:30,rerenderOnResize:false,headerCssClass:null,defaultSortAsc:true,focusable:true,selectable:true};// scroller
	...
	var initialized=false;var $container;var uid="slickgrid_"+Math.round(1000000*Math.random());var self=this;var $focusSink,$focusSink2;var $headerScroller;var $headers;var $headerRow,$headerRowScroller,$headerRowSpacer;var $topPanelScroller;var $topPanel;var $viewport;var $canvas;var $style;var $boundAncestors;var stylesheet,columnCssRulesL,columnCssRulesR;var viewportH,viewportW;var canvasWidth;var viewportHasHScroll,viewportHasVScroll;var headerColumnWidthDiff=0,headerColumnHeightDiff=0,// border+padding
	cellWidthDiff=0,cellHeightDiff=0;var absoluteColumnMinWidth;var numberOfRows=0;var tabbingDirection=1;var activePosX;var activeRow,activeCell;var activeCellNode=null;var currentEditor=null;var serializedEditorValue;var editController;var rowsCache={};var renderedRows=0;var numVisibleRows;var prevScrollTop=0;var scrollTop=0;var lastRenderedScrollTop=0;var lastRenderedScrollLeft=0;var prevScrollLeft=0;var scrollLeft=0;var selectionModel;var selectedRows=[];var plugins=[];var cellCssClasses={};var columnsById={};var sortColumns=[];var columnPosLeft=[];var columnPosRight=[];// async call handles
	var h_editorLoader=null;var h_render=null;var h_postrender=null;var postProcessedRows={};var postProcessToRow=null;var postProcessFromRow=null;// perf counters
	var counter_rows_rendered=0;var counter_rows_removed=0;//////////////////////////////////////////////////////////////////////////////////////////////
	// Initialization
	function init(){$container=$(container);if($container.length<1){throw new Error("SlickGrid requires a valid container, "+container+" does not exist in the DOM.");}// calculate these only once and share between grid instances
	...
	if(!/relative|absolute|fixed/.test($container.css("position"))){$container.css("position","relative");}$focusSink=$("<div tabIndex='"+(options.enableFocusSinkTabStops?"0":"-1")+"' hideFocus style='position:fixed;width:0;height:0;top:0;left:0;outline:0;'></div>").appendTo($container);$headerScroller=$("<div class='slick-header ui-state-default' style='overflow:hidden;position:relative;' />").appendTo($container);$headers=$("<div class='slick-header-columns' style='left:-1000px' />").appendTo($headerScroller);$headers.width(getHeadersWidth());$headerRowScroller=$("<div class='slick-headerrow ui-state-default' style='overflow:hidden;position:relative;' />").appendTo($container);$headerRow=$("<div class='slick-headerrow-columns' />").appendTo($headerRowScroller);$headerRowSpacer=$("<div style='display:block;height:1px;position:absolute;top:0;left:0;'></div>").css("width",getCanvasWidth()+scrollbarDimensions.width+"px").appendTo($headerRowScroller);$topPanelScroller=$("<div class='slick-top-panel-scroller ui-state-default' style='overflow:hidden;position:relative;' />").appendTo($container);$topPanel=$("<div class='slick-top-panel' style='width:10000px' />").appendTo($topPanelScroller);if(!options.showTopPanel){$topPanelScroller.hide();}if(!options.showHeaderRow){$headerRowScroller.hide();}$viewport=$("<div class='slick-viewport' style='width:100%;overflow:auto;outline:0;position:relative;;'>").appendTo($container);$viewport.css("overflow-y",options.autoHeight?"hidden":"auto");//ADDED BY BD - START
	$viewport.mousedown(function(e){if($viewport.is(e.target))trigger(self.onScrollMouseDown,{args:e,grid:self});});$viewport.mouseup(function(e){if($viewport.is(e.target))trigger(self.onScrollMouseUp,{args:e,grid:self});});$viewport.mousewheel(function(e){trigger(self.onMouseWheel,{args:e,grid:self});});//ADDED BY BD - END
	if(options.disableHorizontalOverflow)$viewport.css("overflow-x","hidden");$canvas=$("<div class='grid-canvas' />").appendTo($viewport);$focusSink2=$focusSink.clone().appendTo($container);if(!options.explicitInitialization){finishInitialization();}}function finishInitialization(){if(!initialized){initialized=true;viewportW=parseFloat($.css($container[0],"width",true));// header columns and cells may have different padding/border skewing width calculations (box-sizing, hello?)
	...
	document.selection.empty();}catch(e){}}else if(window.getSelection){var sel=window.getSelection();if(sel&&sel.removeAllRanges){sel.removeAllRanges();}}}function isCellPotentiallyEditable(row,cell){// is the data for this row loaded?
	if(row<getDataLength()&&!getDataItem(row)){return false;}// are we in the Add New row?  can we create new from this cell?
	if(columns[cell].cannotTriggerInsert&&row>=getDataLength()){return false;}// does this cell have an editor?
	if(!getEditor(row,cell)){return false;}return true;}function makeActiveCellNormal(){if(!currentEditor){return;}trigger(self.onBeforeCellEditorDestroy,{editor:currentEditor});currentEditor.destroy();currentEditor=null;if(activeCellNode){var d=getDataItem(activeRow);$(activeCellNode).removeClass("editable invalid");if(d){var column=columns[activeCell];var formatter=getFormatter(activeRow,column);activeCellNode.innerHTML=formatter(activeRow,activeCell,getDataItemValueForColumn(d,column),column,d);invalidatePostProcessingResults(activeRow);}}// if there previously was text selected on a page (such as selected text in the edit cell just removed),
	// IE can't set focus to anything else correctly
	if(navigator.userAgent.toLowerCase().match(/msie/)){clearTextSelection();}getEditorLock().deactivate(editController);}function makeActiveCellEditable(editor){if(!activeCellNode){return;}if(!options.editable){throw"Grid : makeActiveCellEditable : should never get called when options.editable is false";}// cancel pending async call if there is one
	clearTimeout(h_editorLoader);if(!isCellPotentiallyEditable(activeRow,activeCell)){return;}var columnDef=columns[activeCell];var item=getDataItem(activeRow);if(trigger(self.onBeforeEditCell,{row:activeRow,cell:activeCell,item:item,column:columnDef})===false){setFocus();return;}getEditorLock().activate(editController);$(activeCellNode).addClass("editable");// don't clear the cell if a custom editor is passed through
	if(!editor){activeCellNode.innerHTML="";}currentEditor=new(editor||getEditor(activeRow,activeCell))({grid:self,gridPosition:absBox($container[0]),position:absBox(activeCellNode),container:activeCellNode,column:columnDef,item:item||{},commitChanges:commitEditAndSetFocus,cancelChanges:cancelEditAndSetFocus});if(item){currentEditor.loadValue(item);}serializedEditorValue=currentEditor.serializeValue();if(currentEditor.position){handleActiveCellPositionChange();}}function commitEditAndSetFocus(){// if the commit fails, it would do so due to a validation error
	...
	"getEditorLock":getEditorLock,"getEditController":getEditController,// Manual Drag-drop and resize support additions
	"onHeaderRowRendered":new Slick.Event(),"setColumnWidth":setColumnWidth,"getColumnHeaderRow":getColumnHeaderRow,"getColumnHeaders":getColumnHeaders});init();}})(jQuery);}).call(window);/*** EXPORTS FROM exports-loader ***/module.exports=Slick;
